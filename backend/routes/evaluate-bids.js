...
router.post('/', async (req, res) => {
  try {
    const { criteria, bids, procurementDetails } = req.body;

    if (!criteria || !bids || !procurementDetails) {
      return res.status(400).json({ error: "Missing required fields: criteria, bids, or procurementDetails" });
    }
    ...
    return res.status(500).json({ error: error.message || "Failed to evaluate bids." });
  }
});
