    ...
    <div className="py-6 px-4 md:py-8 md:px-8 max-w-6xl mx-auto w-full flex flex-col h-[calc(100vh-100px)]">
      ...
      <div className="mb-6 flex-shrink-0">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-green-500 bg-green-500/10 px-3 py-1.5 rounded-sm border border-green-500/20">
      ...
      <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0">
        ...
        <div className="w-full lg:w-80 flex-shrink-0 flex flex-col gap-6 overflow-y-auto">
          ...
          <div className="bg-surface border border-border p-5 rounded-sm shadow-sm relative overflow-hidden">
            <div className="absolute top-0 left-0 w-1 h-full bg-primary" />
            <h3 className="font-mono text-xs text-text-muted uppercase tracking-widest mb-4">[ YOUR_PROFILE ]</h3>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-sm bg-gray-100 border border-border overflow-hidden shrink-0">
          ...
          <div className="bg-surface border border-primary/30 p-5 rounded-sm shadow-sm relative overflow-hidden">
            ...
            <h3 className="font-mono text-xs text-secondary uppercase tracking-widest mb-4 font-bold flex items-center justify-between">
              [ COUNTERPARTY_INFO ]
              <span className="text-[9px] bg-secondary/10 text-secondary px-1.5 py-0.5 rounded-sm">REVEALED</span>
            </h3>
        ...
        <div className="flex-1 bg-surface border border-border rounded-sm shadow-sm flex flex-col min-h-[400px]">
              ...
              messages.map((msg, i) => {
                ...
                return (
                  <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                    ...
                    <div 
                      className={`max-w-[85%] px-4 py-2.5 rounded-sm text-sm border ${
                        isMe 
          ...
          <div className="p-4 border-t border-border bg-white">
            <form onSubmit={sendMessage} className="flex gap-3">
              <input
                ...
                className="flex-1 bg-surface border border-border px-4 py-3 rounded-sm text-sm font-mono focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
                disabled={sending}
              />
              <button
                type="submit"
                disabled={!input.trim() || sending}
                className="bg-primary hover:bg-primary-dark text-white px-5 py-3 rounded-sm font-mono text-xs font-bold tracking-widest uppercase transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
